...
public class GuestBookService {
	...
	public void update(GuestBookDTO dto) {
		GuestBookEntity entity = gr.findById(dto.getNum())
				.orElseThrow(() -> new EntityNotFoundException(dto.getNum() + "번 글이 존재하지 않습니다."));
	...
	public void recommend(Integer num, String ip) {
		...
		// 2. 추천 이력 저장
		GuestBookRecommendEntity recommendEntity = GuestBookRecommendEntity.builder()
				.id(key)
				.build();
		grr.save(recommendEntity);
...
} // record는 생성자를 만들지 않아도 되는 클래스임. 대신 수정 불가
